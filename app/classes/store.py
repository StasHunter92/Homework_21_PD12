...
from app.exceptions import CapacityLimitError, StoreNotEnoughSpaceError, IncorrectItemError, IncorrectItemCountError, \
    FreeSpaceError


# ----------------------------------------------------------------------------------------------------------------------
# Store class inherited from abstract storage
class Store(Storage):
    ...
    def add(self, title: str, count: int) -> None:
        ...
        if title not in self.get_items():
            if count <= self.get_free_space():
                ...
                raise StoreNotEnoughSpaceError
        else:
            if count <= self.get_free_space():
                self.get_items()[title] += count
            else:
